...
fetch(`http://localhost:3000/paciente/traerDatosPaciente/${datos.id}`)
  .then(response => {
    if (response.ok) {
...
btnEditar.addEventListener("click", function () {
  ...
  if (
    ...
    fetch(`http://localhost:3000/paciente/editarPaciente/${datos.id}`, {
      method: "PUT",
      headers: {
      ...
      .then((response) => {
        ...
        Swal.fire({
          ...
          window.location.assign("http://127.0.0.1:5501/Cliente/datosPersonales.html");
        });
      })
  }
});
